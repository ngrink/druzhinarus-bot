...
export const administrationMenu = new Menu<Context>("admin-menu")
  ...
  .submenu("Рубрики", "admin-category-menu").row()
...
export const tripsMenu = new Menu<Context>("admin-trips-menu")
  ...
  .text("Список рубрик", handlers.showCategoriesHandler).row()
  .text("Добавить рубрику", handlers.addCategoryHandler).row()
  .text("Изменить рубрику", handlers.editCategoryHandler).row()
  .text("Удалить рубрику", handlers.deleteCategoryHandler).row()
  .back("<- Назад");
